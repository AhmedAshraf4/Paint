...
public class Gui {
	...
	private class PaintSurface extends JComponent {
		...
		private DynamicStackBetterShape shapesCurrent = new DynamicStackBetterShape(2);
		private DynamicStackBetterShape shapesUndoNormalObject = new DynamicStackBetterShape(2);
		private DynamicStackInt shapesUndoType = new DynamicStackInt(2);
		private DynamicStackInt shapesRedoType = new DynamicStackInt(2);
		private DynamicStackInt shapesDeletedIndex = new DynamicStackInt(2);
		private DynamicStackBetterShape shapesDeletedObject = new DynamicStackBetterShape(2);
		private DynamicStackInt shapesDeletedIndexRedo = new DynamicStackInt(2);
		private DynamicStackBetterShape shapesUndoMovResOldObject = new DynamicStackBetterShape(2);
		private DynamicStackInt shapesUndoMovResIndex = new DynamicStackInt(2);
		private DynamicStackBetterShape shapesRedoMovResObject = new DynamicStackBetterShape(2);
		private DynamicStackInt shapesRedoMovResIndex = new DynamicStackInt(2);
			...
			this.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					if (tool < 7) {
							...
							if (sliderOpacity.getValue() != 0) { // in case opacity 0 don't add shape object
								if (tool != 0) {
									if (tool == 1 && fill) { // in case line and fill don't add shape object
										...
										shapesCurrent.push(new BetterShape(r, color, fill, sliderStroke.getValue(),
									...
									shapesCurrent.push(new BetterShape(brushPath, color, fill, sliderStroke.getValue(),
						...
						if (!(tool == 1 && fill)) {
							shapesUndoType.push(0);
						}
					} else if (selectedShape != -1) {
						if (tool == 8) {
							Shape r = makeShape(startDrag.x, startDrag.y, endDrag.x, endDrag.y);
							BetterShape old = shapesCurrent.popBetterShapeI(selectedShape);
							shapesUndoMovResOldObject.push(old);
							shapesUndoMovResIndex.push(selectedShape);
							shapesUndoType.push(3);
							shapesCurrent.pushIndex(new BetterShape(r, old.getColor(), old.getFill(), old.getStroke(),
		...
		public void paint(Graphics g) {
			BetterShape[] shapesArray = shapesCurrent.getArray();
		...
		private void undo() {
			int s2 = shapesUndoType.pop();
			if (s2 != -1) {
				selectedShape = -1;
				if (s2 == 0) { // s4 == 1 (normal/copy)
					shapesUndoNormalObject.push(shapesCurrent.pop());
					shapesRedoType.push(0);
				} else if (s2 == 2) { // s2 == 2 (deleted)
					int i = shapesDeletedIndex.pop();
					shapesCurrent.pushIndex(shapesDeletedObject.pop(), i);
					shapesRedoType.push(2);
					shapesDeletedIndexRedo.push(i);
				} else { // s2 == 3 (move/resize)
					int i = shapesUndoMovResIndex.pop();
					shapesRedoMovResObject.push(shapesCurrent.popBetterShapeI(i));
					shapesRedoMovResIndex.push(i);
					shapesCurrent.pushIndex(shapesUndoMovResOldObject.pop(), i);
					shapesRedoType.push(3);
				}
				repaint();
			}
		}
		...
		private void redo() {
			int s4 = shapesRedoType.pop();
			if (s4 != -1) {
				selectedShape = -1;
				if (s4 == 0) { // s4 == 1 (normal/copy)
					shapesCurrent.push(shapesUndoNormalObject.pop());
					shapesUndoType.push(0);
				} else if (s4 == 2) { // s4 == 2 (deleted)
					deleteShape(shapesDeletedIndexRedo.pop());
				} else { // s4 == 3 (move/resize)
					int i = shapesRedoMovResIndex.pop();
					shapesUndoMovResOldObject.push(shapesCurrent.popBetterShapeI(i));
					shapesUndoMovResIndex.push(i);
					shapesCurrent.pushIndex(shapesRedoMovResObject.pop(), i);
					shapesUndoType.push(3);
				}
				repaint();
			}
		...
		private Shape makeShape(int x1, int y1, int x2, int y2) {
			Shape r = selectedShape == -1 ? null : shapesCurrent.getArray()[selectedShape].getShape();
			if (brushPath == null) {
				if (tool == 0) {
		...
		private void deleteShape() {
			if (selectedShape != -1) {
				shapesDeletedObject.push(shapesCurrent.popBetterShapeI(selectedShape));
				shapesUndoType.push(2);
				shapesDeletedIndex.push(selectedShape);
				selectedShape = -1;
				repaint();
			}
		}
		...
		private void deleteShape(int i) {
			shapesDeletedObject.push(shapesCurrent.popBetterShapeI(i));
			shapesUndoType.push(2);
			shapesDeletedIndex.push(i);
			selectedShape = -1;
			repaint();
		...
		private void copyShape() {
			if (selectedShape != -1) {
				...
				BetterShape bs = shapesCurrent.getArray()[selectedShape];
				...
				selectedShape = shapesCurrent.getSize() - 1;
				shapesUndoType.push(0);
				repaint();
			}
		}
		...
		private void clearHistory() {
			while (!shapesUndoNormalObject.isEmpty()) {
				shapesUndoNormalObject.pop();
			}
			while (!shapesRedoType.isEmpty()) {
				shapesRedoType.pop();
			}
			while (!shapesDeletedIndexRedo.isEmpty()) {
				shapesDeletedIndexRedo.pop();
			}
			while (!shapesRedoMovResObject.isEmpty()) {
				shapesRedoMovResObject.pop();
			}
			while (!shapesRedoMovResIndex.isEmpty()) {
				shapesRedoMovResIndex.pop();
			}
		}
